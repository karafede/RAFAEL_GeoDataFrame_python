...
### count how many records for vehicle type
count_idterm = dbGetQuery(conn_HAIG, "SELECT vehtype, COUNT(*)
                                      FROM dataraw 
                                      group by vehtype ")
### count how many idterm for vehicle type
count_vehtype = dbGetQuery(conn_HAIG, "SELECT vehtype, COUNT(*)
                                      FROM idterm_portata 
                                      group by vehtype ")
...
all_breaktime = dbGetQuery(conn_HAIG, "SELECT breaktime_s 
                             FROM route_2019
                                    ")
all_breaktime_pos <- all_breaktime %>%
  mutate(breaktime_m = breaktime_s/60) %>%
  filter(breaktime_m < 10)   ## less than 10 minutes (that is the time of a normal trip)


(nrow(all_breaktime_pos)/nrow(all_breaktime))*100
...
data_GG <- read_csv("route_CATANIA_GG.csv")
# data_GG = dbGetQuery(conn_HAIG, "SELECT *
#                                     FROM \"route_2019_GG\"
#                                     ")
...
data_GG <- data_GG %>%
  select(idterm,
         ...
         Viag_Km,
         Viag_Durata)
# write_csv(data_GG, "route_CATANIA_GG.csv")
...
### rename data
data_GG$vehtype <- gsub(1, "auto", (data_GG$vehtype))
data_GG$vehtype <- gsub(2, "veicoli commerciali", (data_GG$vehtype))
## rename columns
...
# data_FK = dbGetQuery(conn_HAIG, "SELECT route.idtrajectory,
...
# data_FK <- read_csv("route_BRESCIA.csv")
## get month
data_FK <- data_FK %>%
  mutate(month = month(timedate_o))
# unique(data_FK$month)   # 3 11 for Brescia



### only for SALERNO
# data_FK$year <- year(as.Date(data_FK$timedate_o))
# data_FK <- data_FK %>%
#   filter(year == "2019")
# data_FK <- data_FK %>%
#   select(- year)
...
# remove(data_FK_Feb_Nov, data_FK_May_Aug)
# # ---> CATANIA
# data_FK <- data_FK %>%
...
#   mutate(timedate_o = timedate_o - hours(1))
# 
# data_FK <- rbind(data_FK_Nov, data_FK_March)
# remove(data_FK_March, data_FK_Nov)
# data_FK <- data_FK %>%
...
data_FK <- data_FK %>%
  select(- month)
data_FK$deviation_pos_m <- round(data_FK$deviation_pos_m, digits = 0)

## get consecutive unique id for each "idtrajectory"
AAA <- data_FK[order(data_FK$idtrajectory, data_FK$idterm),]
AAA$idtrajectory <- 1:nrow(AAA)

## get consecutuve unique id for each "idterm"
AAA <- transform(AAA,idterm = as.numeric(factor(idterm)))


# AAA$speed_trip = (AAA$tripdistance_m/1000)/(AAA$triptime_s/3600)  ## km/h
# AAA$speed_trip <- round(AAA$speed_trip, digits = 0)
# AAA <- AAA %>%
...
### sample random rows
# BBB <- sample_n(AAA, 500000)
BBB <- AAA %>%
  sample_frac(0.2)

dir <- "D:/Federico/FCDapp"
# saveRDS(BBB, file = paste0(dir, "/", "route_BRESCIA_random.rds"))
# saveRDS(BBB, file = paste0(dir, "/", "route_CATANIA_random.rds"))
# saveRDS(BBB, file = paste0(dir, "/", "route_SALERNO_random.rds"))
saveRDS(BBB, file = paste0(dir, "/", "route_ROMA_random.rds"))
# write_csv(data_FK, "route_BRESCIA.csv")
# write_csv(data_FK, "route_ROMA.csv")
# write_csv(data_FK, "route_SALERNO.csv")
# write_csv(data_FK, "route_CATANIA.csv")


##### --------------------------- ##################
####################################################
### load mapmatching data
dir <- "D:/Federico/FCDapp"
# mapmatching <- readRDS(file = paste0(dir, "/","matched_routes_ROMA_2019_10_09.rds"))
mapmatching <- read_csv(file = paste0(dir, "/","matched_routes_ROMA_2019_10_09.csv"))[-1]
mapmatching <- mapmatching %>% 
  filter(`load(%)` >= 17) 
mapmatching <- mapmatching %>% 
  filter(hr <= 21 & hr >= 6) 
saveRDS(mapmatching, file = paste0(dir, "/", "matched_routes_ROMA_2019_10_09.rds"))


# mapmatching <- readRDS(file = paste0(dir, "/","matched_routes_CATANIA_2019_02_14.rds"))
mapmatching <- read_csv(file = paste0(dir, "/","matched_routes_CATANIA_2019_02_14.csv"))[-1]
mapmatching <- mapmatching %>% 
  filter(`load(%)` >= 17) 
mapmatching <- mapmatching %>% 
  filter(hr <= 21 & hr >= 6) 
saveRDS(mapmatching, file = paste0(dir, "/", "matched_routes_CATANIA_2019_02_14.rds"))


mapmatching <- read_csv(file = paste0(dir, "/","matched_routes_SALERNO_2019_09_19.csv"))[-1]
mapmatching <- mapmatching %>% 
  filter(`load(%)` >= 17) 
mapmatching <- mapmatching %>% 
  filter(hr <= 21 & hr >= 6) 
saveRDS(mapmatching, file = paste0(dir, "/", "matched_routes_SALERNO_2019_09_19.rds"))


mapmatching <- read_csv(file = paste0(dir, "/","matched_routes_BRESCIA_2019_03_14.csv"))[-1]
mapmatching <- mapmatching %>% 
  filter(`load(%)` >= 17) 
mapmatching <- mapmatching %>% 
  filter(hr <= 21 & hr >= 6) 
saveRDS(mapmatching, file = paste0(dir, "/", "matched_routes_BRESCIA_2019_03_14.rds"))
...
p <- ggplot(data = viaggi_10kmh,
    ...
    theme(axis.text.x=element_text(size=13,face="bold", colour = "black")) +
    theme(axis.title.x = element_blank()) +                  # Remove x-axis label
    ylab("(% of trips)") +            # Set y-axis label
    xlab(expression(paste("velocità (km/h)"))) +
...
trips$month <- gsub(10, "ottobre", (trips$month))
...
#### Istogramma di tutti i viaggi con tempi di viaggio ad intervalli di 2 minuti
...
### auto ###
BBB <- NULL
bin_trip = seq(from = 0, to = 80, by = 2)   ## max 2 ore trip = 120 minutes (2 hour trip)

for (i in 1:length(bin_trip)){
  if (i==1){
    AAA <- trips %>%
      filter(vehtype == "auto") %>%
      filter(triptime_min <= bin_trip[i])  %>%
      summarise(counts = length(idtrajectory))
  }else{
    AAA <- trips %>%
       filter(vehtype == "auto") %>%
      filter(triptime_min <= bin_trip[i] & triptime_min >= bin_trip[i-1]) %>%
      summarise(counts = length(idtrajectory))
...
viaggi_5min_auto <- as.data.frame(cbind(bin_trip, BBB))
...
viaggi_5min_commerciali <- as.data.frame(cbind(bin_trip, BBB))
...
## plot
viaggi_5min$bin_trip <- as.factor(viaggi_5min$bin_trip)
viaggi_5min$bin_trip <- as.numeric(viaggi_5min$bin_trip)

p <- ggplot(data = viaggi_5min,
              aes(bin_trip, perc_count, fill = bin_trip)) + guides(fill=FALSE) +
    geom_bar(stat = "identity") + 
    guides(fill=FALSE) +
    facet_wrap(~vehtype) +
    # ylim(0, 15) +
    ...
    theme(axis.title.x = element_blank()) +                  # Remove x-axis label
    ylab("(%)") +            # Set y-axis label
    xlab(expression(paste("tempo di viaggio (minuti)"))) +
...
## summary statistics for trip time #############
...
#### Istogramma di tutti i viaggi con tempi di viaggio ad intervalli di 5 minuti
...
### auto ###
BBB <- NULL
bin_trip = seq(from = 0, to = 140, by = 5)   ## max 2 ore trip = 120 minutes (2 hour trip)
...
### veicoli commerciali ###
BBB <- NULL
bin_stop = seq(from = 0, to = 130, by = 5)   ## max 2 ore trip = 120 minutes (2 hour trip)
...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura 9.** Numero di viaggi per sosta temporale (5 min)."}
...
## convert stop time into minutes
trips <- as.data.frame(trips)
trips$breaktime_min <- (trips$breaktime_s)/60

# ## quick check -----------------------
# ## tempo di ssta
# ggplot(trips, aes(breaktime_min)) +
# geom_density()
...
### auto ###
BBB <- NULL
bin_stop = seq(from = 0, to = 180, by = 10)   

for (i in 1:length(bin_stop)){
  if (i==1){
    AAA <- trips %>%
      filter(vehtype == "auto") %>%
      filter(breaktime_min <= bin_stop[i])  %>%
      summarise(counts = length(idtrajectory))
  }else{
    AAA <- trips %>%
       filter(vehtype == "auto") %>%
      filter(breaktime_min <= bin_stop[i] & breaktime_min >= bin_stop[i-1]) %>%
      summarise(counts = length(idtrajectory))
...
### veicoli commerciali ###
BBB <- NULL
bin_stop = seq(from = 0, to = 180, by = 10)   ## max 2 ore trip = 120 minutes (2 hour trip)
...
p <- ggplot(data = viaggi_5min,
              aes(bin_stop, perc_count, fill = vehtype)) + guides(fill=FALSE) +
    geom_bar(stat = "identity") + 
    guides(fill=FALSE) +
    facet_wrap( ~ vehtype, ncol = 1) +
    # ylim(0, 30) +
    ...
    theme(axis.title.y = element_text(face="bold", colour="black", size=14),
          axis.text.y  = element_text(angle=0, vjust=0.5, size=14)) +
    # scale_x_continuous(breaks=c(0, 5, 10, 15, 20)) +
    ggtitle("Percentuale di soste per durata temporale") + 
    theme(plot.title = element_text(lineheight=.8, face="bold"))
  p
...
#### Istogramma di tutti i viaggi con tempi di sosta ad intervalli di 10 minutes
# ## quick check -----------------------
# ## tempo di sosta
# ggplot(trips, aes(breaktime_min)) +
# geom_density()
...
trips <- trips %>%
  filter(breaktime_min < 24*60)   # D_Sosta_Dur in units of HOURS

### auto ###
BBB <- NULL
bin_stop = seq(from = 0, to = 1000, by = 10)   
...
### veicoli commerciali ###
BBB <- NULL
bin_stop = seq(from = 0, to = 1000, by = 10)   ## max 2 ore trip = 120 minutes (2 hour trip)
...
p <- ggplot(data = viaggi_5min,
    ...
    xlab(expression(paste("tempo di sosta (minuti)"))) +
    theme(axis.title.x = element_text(face="bold", colour="black", size=13),
          axis.text.x  = element_text(angle=90, vjust=0.5, size=5, hjust = 0.5)) + 
    theme(axis.title.y = element_text(face="bold", colour="black", size=14),
          axis.text.y  = element_text(angle=0, vjust=0.5, size=14)) +
    # scale_x_continuous(breaks=c(0, 5, 10, 15, 20)) +
    ggtitle("Percentuale di soste per durata temporale") + 
  ...
  ggplot() + 
  geom_density(data=trips, aes(x = breaktime_min, group=vehtype, color=vehtype), adjust=2) +
...
## convert stop time into minutes
data_GG <- as.data.frame(data_GG)
trips <- data_GG %>%
  filter(D_Sosta_Dur < 24)   # D_Sosta_Dur in units of HOURS
trips$D_Sosta_Dur <- (trips$D_Sosta_Dur)*60

# ## quick check -----------------------
# ## tempo di sosta
ggplot(trips, aes(D_Sosta_Dur)) +
geom_density()
...
for (i in 1:length(bin_stop)){
  if (i==1){
    AAA <- trips %>%
      filter(vehtype == "auto") %>%
      filter(D_Sosta_Dur <= bin_stop[i])  %>%
      summarise(counts = length(Id_Viaggi))
  }else{
    AAA <- trips %>%
       filter(vehtype == "auto") %>%
      filter(D_Sosta_Dur <= bin_stop[i] & D_Sosta_Dur >= bin_stop[i-1]) %>%
      summarise(counts = length(Id_Viaggi))
