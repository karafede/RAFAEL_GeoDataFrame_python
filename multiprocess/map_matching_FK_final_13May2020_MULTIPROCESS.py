...
def func(arg):
    ...
    for idx_trip, trip in enumerate(all_TRIPS):
        ...
        if len(viasat) > 5:
            ...
            # my_map.save("MULTIPROCESS_matched_route_VIASAT_" + '_' + today + ".html")
                ...
                if len(all_matched_edges) > 1:
                    ...
                    if len(edges_matched_route_GV) > 0:
                        ...
                        try:
                            ...
                            final_map_matching_table_GV.to_sql("mapmatch_MULTIPROC_temp", con=connection, schema="public",
                                               if_exists='append')
                            connection.close()
                        except KeyError:
                    ...
                    try:
                        ...
                        ## save data (GeoJSON)
                        # with open('MULTIPROCESS_all_EDGES_' + '_' + today + '.geojson', 'w') as f:
                        #     f.write(all_EDGES.to_json())

                        ## add plot in Folium map
                        # save first as geojson file
                        # edges_matched_route.geometry.to_file(filename='MULTIPROCESS_matched_route_' + '_' + today + '.geojson',
                        #                                      driver='GeoJSON')
                        ## MAP ###
                        # folium.GeoJson('MULTIPROCESS_matched_route_' + '_' + today + '.geojson').add_to((my_map))
                        # my_map.save("MULTIPROCESS_matched_route_VIASAT_" + '_' + today + ".html")
                        # save last track ID and index (to be used in case the query stops)

                        # with open("last_track_ID_MULTIPROCESS.txt", "w") as text_file:
