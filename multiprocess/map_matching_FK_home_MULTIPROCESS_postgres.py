                        ...
                        if len(edge) > 0:
                            ...
                            if len(all_matched_edges) > 1:
                                ...
                                try:
                                    ...
                                    with open("all_edges{}.geojson".format(last_track_idx), 'w') as fff:
                                        fff.write('last_track_idx\n')
                                        # ff.write(str(len(all_EDGES)) + '\n')
                                        # ff.write("inside_AAAAAAAAAAA" + '\n')
                                        fff.write(all_EDGES.to_json())

                                    with open("file{}.txt".format(last_track_idx), 'w') as ff:
                                        ff.write('last_track_idx\n')
                                        # ff.write(str(len(all_EDGES)) + '\n')
                                        ff.write("inside_AAAAAAAAAAA" + '\n')

                                    ## add plot in Folium map
                                    ## save first as geojson file
                                    edges_matched_route.geometry.to_file(filename='MULTIPROCESS_matched_route_' + DATE + '_' + today + '.geojson',
                                                                         driver='GeoJSON')
                                    folium.GeoJson('MULTIPROCESS_matched_route_' + DATE + '_' + today + '.geojson').add_to((my_map))
                                    my_map.save("MULTIPROCESS_matched_route_VIASAT_"+ DATE + '_' + today + ".html")
...
# https://stackoverflow.com/questions/49226370/initiating-multiprocesses-pool-in-a-for-loop-in-python
#     def handler():
#         p = mp.Pool(5)
#         # r = p.map(f, data)
#         # r = p.map(func, [(last_track_idx, track_ID) for last_track_idx, track_ID in enumerate(all_ID_TRACKS)])
#         r = p.map(func, [(last_track_idx, track_ID) for last_track_idx, track_ID in enumerate(all_ID_TRACKS)])
#         p.join()
#         return r
    ...
    #     pool = mp.Pool(5)
    #     tmp = pool.map(func, [(last_track_idx, track_ID) for last_track_idx, track_ID in enumerate(all_ID_TRACKS)])
    #     # Sort the output
    #     mp_out = []
    #     for t in tmp:
    #         mp_out.extend(t)
    ...
    if __name__ == '__main__':
        ...
        # results = [pool.apply(func, args=[last_track_idx, track_ID], for last_track_idx, track_ID in enumerate(all_ID_TRACKS)]
        results = pool.map(func, [(last_track_idx, track_ID) for last_track_idx, track_ID in enumerate(all_ID_TRACKS)])
        pool.close()
