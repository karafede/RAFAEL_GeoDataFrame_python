...
os.chdir('D:/ENEA_CAS_WORK/SENTINEL/viasat_data/data')
os.getcwd()
## load grafo
...
def func(track_ID):
    ...
    for idx_trip, trip in enumerate(all_TRIPS):
        ...
        if len(viasat) > 4:
            ...
            ## https: // github.com / geopandas / geopandas / pull / 1498
            viasat_extent = pd.DataFrame(viasat_extent, copy=False)
            ...
            if len(grafo) > 0:
                ...
                # crs = {'init': 'epsg:4326'}
                # viasat_gdf = gpd.GeoDataFrame(viasat, crs=crs, geometry=geometry)
                ## use version of module pyproj == 2.5
                viasat_gdf = gpd.GeoDataFrame(viasat, crs="epsg:4326", geometry=geometry)
                ...
                ### ------ >>>>> #################################
                ## define a new variable with u, v for the edges
                df_gdf_edges = gdf_edges
                df_gdf_edges = pd.DataFrame(df_gdf_edges)
                df_gdf_edges.reset_index(inplace = True)
                # gdf_edges_df = pd.DataFrame(gdf_edges)
                # gdf_edges_df.reset_index(inplace = True)
                ### ------ >>>>> #################################
                for streets in df_gdf_edges.itertuples(index=True):
                ...
                if len(edge) > 0:
                    ...
                    if len(track_list) > 1:
                        ...
                        for i in range(len(track_list)):
                            ...
                            if len(max_prob_node) != 0:
                                distance_to_track_i = ox.distance.great_circle_vec(lat1 = grafo.nodes[max_prob_node[-1]]['y'],
                                ...
                                distance_to_track_i1 = ox.distance.great_circle_vec(lat1 = grafo.nodes[max_prob_node[-1]]['y'],
                                                                           lng1 = grafo.nodes[max_prob_node[-1]]['x'],
                                                                           lat2=float(viasat.latitude[
                                                                                          viasat.ID == track_list[
                                                                                              i + 1]]),
                    ...
                    if len(all_matched_edges) > 1:
                        ...
                        edges_matched_route = pd.merge(df_nodes, df_gdf_edges, on=['u', 'v'], how='left')
