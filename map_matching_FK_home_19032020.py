...
all_ID_TRACK = list(all_VIASAT_IDterminals.idterm.unique())
...
sample_data_timerange = pd.read_sql_query(
    ''' SELECT *
        FROM public.dataraw
        WHERE timedate BETWEEN '2019-04-15 12:24:27' AND '2019-04-15 12:40:00' ''', conn)

# select by IDterm
track = pd.read_sql_query(
    ''' SELECT *
        FROM public.dataraw
        WHERE timedate BETWEEN '2019-04-15 06:34:00' AND '2019-04-15 06:44:00' 
        AND idterm = '%s' ''' % track_ID, conn)

track = pd.read_sql_query(
    ''' SELECT *
        FROM public.dataraw
        WHERE timedate > '2019-04-15 06:34:00' 
        AND idterm = '%s' ''' % track_ID, conn)

range_datetime = pd.read_sql_query(
    ''' SELECT min(timedate) as minimum_time,  max(timedate) as maximum_time
        FROM public.dataraw ''', conn)
...
# grafo_bis = ox.load_graphml(file_graphml)
# ox.plot_graph(grafo)


# load Viasat data
# viasat_data = "viasat_data_5428266.csv"  #OK
# viasat_data = "viasat_data_2739519.csv"  #OK - odometer is all zeros
# viasat_data = "viasat_max_data_short.csv"  #OK - odometer is all zeros
# viasat_data = "viasat_data_3511558.csv"  #OK
# viasat_data = "viasat_data_3219572.csv"  #OK
# viasat_data = "viasat_data_4452493.csv"  # OK
# viasat_data = 'viasat_data_2754384.csv' #OK
# viasat_data = 'viasat_data_4094485.csv'  #OK
# viasat_data = 'viasat_data_4298830.csv'   # OK
# viasat_data = 'viasat_data_5410997.csv'   #OK
# viasat_data = 'viasat_data_5220538.csv'   #OK
# viasat_data = 'viasat_data_4012647.csv'   #OK
# viasat_data = 'viasat_data_4172466.csv'  #OK
...
viasat['datetime'] = viasat['datetime'].astype('datetime64[ns]')
# date
viasat['date'] = viasat['datetime'].apply(lambda x: x.strftime("%Y-%m-%d"))
# hour
viasat['hour'] = viasat['datetime'].apply(lambda x: x.hour)
# minute
viasat['minute'] = viasat['datetime'].apply(lambda x: x.minute)
# seconds
viasat['seconds'] = viasat['datetime'].apply(lambda x: x.second)
...
# grafo = ox.graph_from_polygon(viasat_extent.geometry[0])
...
# remove viasat data with 'odometer' == 0
# viasat = viasat[viasat['odometer'] != 0]
...
buffer = viasat_gdf.buffer(buffer_diam)  # 25 meters # this is a geoseries (0.00025)
# buffer.plot()
# make a dataframe
...
def great_circle_track(u):
    # Calculate the great circle distance between two points from viasat data (odometer)
    u_track = nodes_dict.get(u)[0][2]
    v_track = u_track+1
    if v_track <= max(viasat.ID):
        distance =int((viasat[viasat['ID'] == v_track]).odometer) - int((viasat[viasat['ID'] == u_track]).odometer)
    ...
    for i in range(len(track_list)):
        ...
        for u in adjacency_list[track_list[i]]:
            for v in adjacency_list[track_list[i+1]]:
                ...
                distance_VIASAT = int((viasat[viasat['ID'] == track_list[i + 1]]).odometer) - int(
                    (viasat[viasat['ID'] == track_list[i]]).odometer)
                ...
                try:
                    if u != v:
                        ...
                        # point = (lat, lon)
                        # nearest_node = ox.get_nearest_node(grafo, point, return_dist=True)
