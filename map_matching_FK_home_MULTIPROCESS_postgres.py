...
os.chdir('D:\\ENEA_CAS_WORK\\Catania_RAFAEL\\multiprocess')
os.getcwd()
...
for idx, row in unique_DATES.iterrows():
    ...
    def func(arg):
            ...
            if len(row) > 0:
                ...
                for idx, j in enumerate(row):
                    ...
                    if len(viasat_data) > 5:
                        ...
                        #     'tertiary_link"]')
                        filter = (
                            '["highway"!~"living_street|abandoned|steps|construction|service|'
                        ...
                        my_map.save("MULTIPROCESS_matched_route_VIASAT_" + DATE + '_' + today + ".html")
                        ...
                        buffer.to_file(filename='MULTIPROCESS_buffer_viasat.geojson', driver='GeoJSON')
                        # folium.GeoJson('MULTIPROCESS_buffer_viasat.geojson').add_to((my_map))
                        ...
                        for streets in gdf_edges.itertuples(index=True):
                            for via_buff in buffer_viasat.itertuples(index=True):
                                if streets.geometry.intersects(via_buff.geometry) is True:
                                    ...
                                    distance = (Point(viasat[['longitude', 'latitude']].iloc[via_buff.Index]).distance(
                                        streets.geometry)) * 100000  # roughly meter conversion
                        ...
                        if len(edge) > 0:
                            ...
                            keys = ID_TRACK
                            for track in keys:
                                df_edges_dict[track] = df_edges[['u', 'v']][
                                    df_edges['buffer_ID'] == track].values.tolist()
                            print(df_edges_dict)
                                ...
                                for i in range(len(track_list)):
                                    ...
                                    for u in adjacency_list[track_list[i]]:
                                        for v in adjacency_list[track_list[i + 1]]:
                                            ...
                                            distance_VIASAT = int(
                                                (viasat[viasat['ID'] == track_list[i + 1]]).progressive) - int(
                                            ...
                                            time_VIASAT = int(
                                                (viasat[viasat['ID'] == track_list[i + 1]]).path_time) - int(
                                            ...
                                            timedate_track[u] = ((viasat[
                                                viasat['ID'] == track_list[i]]).timedate).to_string()[4:23]
                                            ...
                                            try:
                                                if u != v:
                                                    shortest_path[u] = nx.shortest_path_length(grafo, u, v,
                                                                                               weight='length') / 1000
                                                    short_path = nx.shortest_path_length(grafo, u, v,
                                                                                         weight='length') / 1000
                                                    SHORT_PATH.append(short_path)
                                                    if shortest_path[u] <= distance_VIASAT:
                                                        print('===== KEEP ========')
                                                        print('#u:', u, '#v:', v, 'shortest_path:',
                                                              nx.shortest_path_length(grafo, u, v,
                                                                                      weight='length') / 1000,
                                                              'distance_VIASAT:', distance_VIASAT)
                                        ...
                                        while MAX_trans_key not in adjacency_list[track_list[i + 1]]:
                                            ...
                                            for u in adjacency_list[track_list[i]]:
                                                for v in adjacency_list[track_list[i + 1]]:
                                                    ...
                                                    if int((viasat[viasat['ID'] == track_list[i + 1]]).speed) == 0:
                                                        speed_VIASAT = int(
                                                            (viasat[viasat['ID'] == track_list[i]]).speed)
                                                    elif int((viasat[viasat['ID'] == track_list[i]]).speed) == 0:
                                                        speed_VIASAT = int(
                                                            (viasat[viasat['ID'] == track_list[i + 1]]).speed)
                                                    else:
                                                        speed_VIASAT = (int(
                                                            (viasat[viasat['ID'] == track_list[i + 1]]).speed) + int(
                                                            (viasat[viasat['ID'] == track_list[i]]).speed)) / 2
                                            ...
                                            if MAX_trans_value != 0:
                                                ...
                                                if MAX_emiss_key in DISTANCES_dict.keys():
                                                    if MAX_trans_key != MAX_emiss_key and DISTANCES_dict[
                                    ...
                                    if track_list[i] == 0:
                                        if MAX_trans_key == 0 or MAX_emiss_key == 0:
                                            ...
                                            nearest_node = min((u, v), key=lambda n: ox.great_circle_vec(lat, lon,
                                                                                                         grafo.nodes[n][
                                                                                                             'y'],
                                                                                                         grafo.nodes[n][
                                                                                                             'x']))
                                            ...
                                            distance_nearest_to_track = ox.great_circle_vec(
                                                lat1=grafo.nodes[nearest_node]['y'],
                                    ...
                                    if len(max_prob_node) != 0:
                                        distance_to_track_i = ox.great_circle_vec(
                                            lat1=grafo.nodes[max_prob_node[-1]]['y'],
                                        ...
                                        distance_to_track_i1 = ox.great_circle_vec(
                                            lat1=grafo.nodes[max_prob_node[-1]]['y'],
                                            lng1=grafo.nodes[max_prob_node[-1]]['x'],
                                            lat2=float(viasat.latitude[
                                                           viasat.ID == track_list[
                                        ...
                                        if np.mean(dists) <= (distance_VIASAT / 2):
                                            ...
                                            nn0 = min((u0, v0),
                                                      key=lambda n: ox.great_circle_vec(lat0, lon0, grafo.nodes[n]['y'],
                                                                                        grafo.nodes[n]['x']))
                                            max_prob_node.append(nn0)
                                            nn1 = min((u1, v1),
                                                      key=lambda n: ox.great_circle_vec(lat1, lon1, grafo.nodes[n]['y'],
                                                                                        grafo.nodes[n]['x']))
                            ...
                            for i in range(len(max_prob_node) - 2):
                                ...
                                if (([max_prob_node[1:(len(max_prob_node) - 1)][i]]) not in df_edges.values[:,
                                                                                            [0]]) and (
                                        ([max_prob_node[1:(len(max_prob_node) - 1)][i]]) not in df_edges.values[:,
                                                                                                [1]]):
                            ...
                            if len(all_matched_edges) > 1:
                                ...
                                if len(filter_edge) != 0:
                                    ...
                                    selected_edges = edges_matched_route[
                                        edges_matched_route.u.isin(list(pd.to_numeric(filter_edge.u)))]
                                ...
                                if len(filter_edge) > 1:
                                    ...
                                    if len(selected_edges) == 1:
                                        ...
                                        # filter row in 'edges_matched_route' with index == idx_edge
                                        # edges_matched_route = edges_matched_route[edges_matched_route.index != idx_edge]
                                        edges_matched_route = edges_matched_route[
                                            ~edges_matched_route.index.isin(idx_edge)]
                                ...
                                distance_edges = pd.DataFrame.from_dict(distance_between_points,
                                ...
                                try:
                                    ...
                                    time_dist_speed_edges.columns = ['u', 'time', 'distance', 'speed', 'hour',
                                                                     'timedate']
                                    # merge "time_track and distances" with the "edges_matched_route"
                                    edges_matched_route = pd.merge(edges_matched_route, time_dist_speed_edges, on=['u'],
                                                                   how='left')
                                    ...
                                    edges_matched_route.loc[
                                        set(edges_matched_route.index) - set(
                                            edges_matched_route_bool.index), 'time'] = np.nan
                                    edges_matched_route.loc[
                                        set(edges_matched_route.index) - set(
                                            edges_matched_route_bool.index), 'distance'] = np.nan
                                    edges_matched_route.loc[
                                        set(edges_matched_route.index) - set(
                                            edges_matched_route_bool.index), 'speed'] = np.nan
                                    edges_matched_route.loc[
                                        set(edges_matched_route.index) - set(
                                            edges_matched_route_bool.index), 'hour'] = np.nan
                                    edges_matched_route.loc[
                                        set(edges_matched_route.index) - set(
                                            edges_matched_route_bool.index), 'timedate'] = np.nan
                                    ...
                                    with open('MULTIPROCESS_all_EDGES_' + DATE + '_' + today + '.geojson', 'w') as f:
                                        f.write(all_EDGES.to_json())

                                    ## add plot in Folium map
                                    ## save first as geojson file
                                    edges_matched_route.geometry.to_file(
                                        filename='MULTIPROCESS_matched_route_' + DATE + '_' + today + '.geojson',
                                        driver='GeoJSON')
                                    folium.GeoJson(
                                        'MULTIPROCESS_matched_route_' + DATE + '_' + today + '.geojson').add_to(
                                        (my_map))
                                    my_map.save("MULTIPROCESS_matched_route_VIASAT_" + DATE + '_' + today + ".html")
                                    # save last track ID and index (to be used in case the query stops)
                                    # with open("last_track_ID_bis.txt", "w") as text_file:
        ...
        return [all_EDGES, edges_matched_route]
    ...
    if __name__ == '__main__':
        ...
        # https://gis.stackexchange.com/questions/140904/how-to-write-to-a-single-table-via-python-multiprocessing
        new_rows = pool.map(func, [(last_track_idx, track_ID) for last_track_idx, track_ID in enumerate(all_ID_TRACKS)])
