...
#             SELECT * FROM public.dataraw WHERE date(timedate) = '%s' ''' % DATE, conn)

# QUERY to select date and track ID
# selected_VIASAT_track = pd.read_sql_query('''
#            SELECT * FROM public.dataraw
#            WHERE date(timedate) = %s
#            AND idterm = %s ''', conn, params=(DATE, track_ID))
...
# gdf_nodes, gdf_edges = ox.graph_to_gdfs(grafo)
        ...
        if len(viasat_data) > 2:
            fields = ["longitude", "latitude", "progressive"]
            ...
            my_map.save("matched_route_02032020.html")
            ...
            if len(edge) > 0:
                ...
                if len(all_matched_edges) > 1:
                    ...
                    with open('all_EDGES.geojson', 'w') as f:
